...
前端位于 `frontend/`，使用 React 19 + Vite 6 + TypeScript 5.7 构建，并引入以下现代中后台技术栈：

- **路由**：`wouter` 提供基于 hooks 的轻量路由。
- **状态管理**：`zustand` 集中管理 dashboard 数据、选择状态、全局提示。
- **样式**：`Tailwind CSS v4` 原子类 + shadcn/ui 组件库，组件级隔离，避免全局类名冲突。
- **图标**：`lucide-react`。

当前视图包括：

- **仪表盘**：综合工作流指挥中心，展示 4 阶段状态、趋势洞察 TOP 榜单、内容工作室三平台预览、平台适配器概览和运行日志时间线。
- **趋势洞察**：筛选源站、切换话题、触发 RSS 采集、查看趋势详情。
- **内容工作室**：左侧资料池多选、中部文章阅读与 AI 分析、右侧三平台版本切换与内容生成。
- **发布管理**：平台适配器状态、发布预览/执行、今日头条账号配置与微头条发布、外部仓库同步。
- **运行日志**：按类型筛选、查看采集/生成/分析/预览/发布/账号/系统事件详情。
